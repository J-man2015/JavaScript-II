...
// ==== Challenge 2: Use .map() ==== returns a new array, doesnt manulate original array
// The event director needs to have all the runners' first names in uppercase because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`. This array will contain just strings.


const newName = runners.map(function(items) {
...
// ==== Challenge 3: Use .filter() ====similar to .map except it filters out results in new array, 
// The large shirts won't be available for the event due to an ordering issue. We need a filtered version of the runners array, containing only those runners with large sized shirts so they can choose a different size. This will be an array of objects.

const runnersLargeSizeShirt = runners.filter(function(items) {
...
const ticketPriceTotal = runners.reduce(function(accumulator, items) {
...
// Problem 1
const lowerCaseName = runners.map(function(items) {

  return items.first_name.toLowerCase();

});
console.log(lowerCaseName);
// Problem 2
const runnersMediumSizeShirt = runners.filter(function(items) {

  return items.shirt_size === "M";

});

console.log(runnersMediumSizeShirt);
// Problem 3
const donationsArr = runners.map(function(items) {

  return items.donation 
